...
#define CRSF_TIME_NEEDED_PER_FRAME_US   1000 // 700 ms + 400 ms for potential ad-hoc request
#define CRSF_TIME_BETWEEN_FRAMES_US     2000//4000 // 4 ms 250Hz
#define CRSF_PAYLOAD_OFFSET offsetof(crsfFrameDef_t, type)
#define CRSF_MAX_CHANNEL 16
#define CRSF_FRAME_SIZE_MAX 64
#define SERIAL_BAUDRATE 400000//400000
...
void loop()
{
  ...
  if (currentMicros > crsfTime) {
      ...
      Serial2.write(crsfPacket, CRSF_PACKET_SIZE);            
      crsfTime = currentMicros + CRSF_TIME_BETWEEN_FRAMES_US;
  }

  if(Serial.available() > 24) {
      //sprintf(control, "%05d", Serial.read());
      control = Serial.readStringUntil(':');
      rcChannels[0] = map(control.substring(0,3).toInt(), 0, 800, CRSF_CHANNEL_MIN, CRSF_CHANNEL_MAX);
      rcChannels[1] = map(control.substring(3,6).toInt(), 0, 800, CRSF_CHANNEL_MIN, CRSF_CHANNEL_MAX);
      rcChannels[2] = map(control.substring(6,9).toInt(), 0, 800, CRSF_CHANNEL_MIN, CRSF_CHANNEL_MAX);
      rcChannels[3] = map(control.substring(9,12).toInt(), 0, 800, CRSF_CHANNEL_MIN, CRSF_CHANNEL_MAX);
      rcChannels[4] = map(control.substring(12,15).toInt(), 0, 800, CRSF_CHANNEL_MIN, CRSF_CHANNEL_MAX);
      rcChannels[5] = map(control.substring(15,18).toInt(), 0, 800, CRSF_CHANNEL_MIN, CRSF_CHANNEL_MAX);
      rcChannels[6] = map(control.substring(18,21).toInt(), 0, 800, CRSF_CHANNEL_MIN, CRSF_CHANNEL_MAX);
